...
async def predict_ticket(request: TicketRequest):
    ...
    try:
        ...
        # VALIDATE: Check if ticket is IT-related
        is_it_ticket, rejection_message = predictor.is_it_related(title, description)
        if not is_it_ticket:
            raise HTTPException(
                status_code=400,
                detail={
                    "error": "Non-IT Ticket Detected",
                    "message": rejection_message,
                    "recommendations": [
                        "If this is a facilities issue (water, HVAC, furniture), contact Facilities Management",
                        "If this is an HR issue (payroll, leave, benefits), contact Human Resources",
                        "If this is truly an IT issue, please rephrase with more technical details"
                    ],
                    "ticket_id": ticket_id
                }
            )
