...
### Health Check
```bash
curl https://itsm-ai-api.hf.space/health
```

### Predict Ticket Classification
```bash
curl -X POST https://itsm-ai-api.hf.space/predict \
...
```bash
curl -X POST https://itsm-ai-api.hf.space/find-similar \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://itsm-ai-api.hf.space/search-kb \
  -H "Content-Type: application/json" \
  -d '{
...
- **Swagger UI**: https://itsm-ai-api.hf.space/docs
- **ReDoc**: https://itsm-ai-api.hf.space/redoc
